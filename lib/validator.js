...
/** Mirrors TITLE_CLASSES in plugins/remark-mdd-text-formatting.js. */
const TITLE_HEADING_CLASSES = new Set(['invoice-title', 'contract-title'])
...
function parseDirectiveStructure(content) {
  ...
  return { directives, orphanedEndMarkers, nestingViolations, unknownDirectives }
}
...
function computeHeadingIds(content) {
  ...
  for (const { text, inFence } of annotateLines(content)) {
    ...
    const classMatch = heading[2].match(/\{\.([^}]+)\}\s*$/)
    const title = heading[2].replace(/\s*\{\.[^}]+\}\s*$/, '').trim()

    // Title headings (invoice-title, contract-title) are neither numbered nor
    // counted by the renderer; they only get their slug id.
    if (classMatch && TITLE_HEADING_CLASSES.has(classMatch[1].trim())) {
