...
// Static JSON imports (import attributes, Node >=24) rather than `fs.readFileSync` of a path resolved
// via `fileURLToPath(import.meta.url)`. This keeps the validator browser/bundler-safe: it has no
// `node:fs`/`node:path`/`node:url` dependency, so it loads and runs in a renderer (e.g. Tauri/Vite)
// as well as in Node. The schema files still ship in the package (`files: schema/**/*.json`).
import documentTypeRequirements from '../schema/document-type-requirements.json' with { type: 'json' }
import mddDocumentSchema from '../schema/mdd-document.schema.json' with { type: 'json' }
...
function loadDocumentTypeRequirements() {
  ...
  requirementsCache = documentTypeRequirements
  return requirementsCache
}
...
export const ERROR_CODES = {
  ...
  DIRECTIVE_NOT_BLOCK_START: 'DIRECTIVE_NOT_BLOCK_START',
...
/**
 * Lines after which Markdown starts a new block, so a directive opener on the
 * next line is parsed as its own paragraph by the renderer: ATX headings,
 * thematic breaks / setext underlines / frontmatter delimiters, and fence
 * delimiters. Any other non-blank line (prose, list items, table rows, a
 * previous directive's `::`) continues the current paragraph, and the renderer
 * then sees the opener as ordinary text.
 */
const BLOCK_TERMINATOR = /^(?:#{1,6}(?:\s|$)|(?:-{3,}|\*{3,}|_{3,}|={3,})\s*$|`{3,}|~{3,})/u

function startsBlock(previousLine) {
  return previousLine === '' || BLOCK_TERMINATOR.test(previousLine)
}
...
function parseDirectiveStructure(content) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (inlineDirectiveType) {
      if (!startsBlock(i > 0 ? lines[i - 1].trim() : '')) {
        notBlockStart.push({ type: inlineDirectiveType, line: lineNumber })
      }
      if (currentDirective) {
    ...
    if (blockDirectiveType) {
      if (!startsBlock(i > 0 ? lines[i - 1].trim() : '')) {
        notBlockStart.push({ type: blockDirectiveType, line: lineNumber })
      }
      if (currentDirective) {
    ...
    const unknownDirectiveMatch =
      trimmedLine.match(/^::([a-z][a-z0-9-]*)(?:\s*::)?$/u) ??
      trimmedLine.match(/^:::\s*([a-z][a-z0-9-]*)(?:\s*:::)?$/u)
    if (unknownDirectiveMatch) {
      unknownDirectives.push({ type: unknownDirectiveMatch[1], line: lineNumber })
      continue
    }
  ...
  return { directives, orphanedEndMarkers, nestingViolations, unknownDirectives, notBlockStart }
}
...
export function validateDirectives(directives) {
  ...
  for (const unknownDirective of parsedDirectiveState.unknownDirectives ?? []) {
    errors.push(
      createError(
        'error',
        ERROR_CODES.UNKNOWN_DIRECTIVE,
        `Unknown directive ::${unknownDirective.type} at line ${unknownDirective.line}`,
        { line: unknownDirective.line, directive: unknownDirective.type },
        'Use a directive type defined by the MDD schema',
      ),
    )
  }
  ...
  for (const opener of parsedDirectiveState.notBlockStart ?? []) {
    errors.push(
      createError(
        'error',
        ERROR_CODES.DIRECTIVE_NOT_BLOCK_START,
        `Directive ::${opener.type} at line ${opener.line} must start a new block; the line above it is not blank, so the renderer treats the opener as paragraph text`,
        { line: opener.line, directive: opener.type },
        `Insert a blank line before ::${opener.type}`,
      ),
    )
  }
...
/**
 * Count the tables and images the text-formatting plugin will anchor as
 * `table-N` / `figure-N` (a table is a run of consecutive `|` lines outside
 * code fences; a figure is a Markdown image).
 *
 * @param {string} content
 * @returns {{ tables: number, figures: number }}
 */
function countReferenceTargets(content) {
  let tables = 0
  let figures = 0
  let inTable = false
...
/**
 * Validate internal `@section-N`, `@table-N` and `@figure-N` references
 * against the anchors the renderer generates: heading ids from section
 * numbering, and tables/images numbered in document order. References inside
 * fenced code blocks are ignored.
 *
 * @param {string} content
 * @returns {{ errors: Array, warnings: Array }}
...
export function validateReferences(content) {
  ...
  const targets = countReferenceTargets(content)
  ...
  for (const { text, inFence, number } of annotateLines(content)) {
    ...
    while ((match = referencePattern.exec(text)) !== null) {
      ...
      let suggestion
      if (refType === 'section') {
        resolves = headingIds.has(`section-${refNumber}`)
        suggestion = 'Add a heading that resolves to this section number, or fix the reference'
      } else if (refType === 'table') {
        resolves = Number(refNumber) >= 1 && Number(refNumber) <= targets.tables
        suggestion = `The document has ${targets.tables} table(s); tables are numbered in order of appearance`
      } else if (refType === 'figure') {
        resolves = Number(refNumber) >= 1 && Number(refNumber) <= targets.figures
        suggestion = `The document has ${targets.figures} image(s); figures are numbered in order of appearance`
      } else {
        continue
      }
      if (!resolves) {
        warnings.push(
          createError(
            ...
            `Internal reference @${refType}-${refNumber} at line ${number} points to a non-existent ${refType}`,
            { line: number },
            suggestion,
          ),
        )
